          ...
          {projects.slice(0, 5).map((project, index) => (
            ...
            >
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-semibold ${
                ...
                {getInitials(project.client.company_name || project.client.name)}
              </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-0.5">
                {currentProject?.client.company_name || currentProject?.client.name} • {currentProject?.progress}% completado
              </p>
            </div>
                ...
                currentProject.team.map((member, i) => (
                  <div key={member.id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-muted hover:bg-border transition-all cursor-pointer">
                    <Avatar size="xs">
                      ...
                      <AvatarFallback variant={getAvatarColor(i)}>{getInitials(member.name)}</AvatarFallback>
                    </Avatar>
                    <span className="text-sm font-medium text-foreground">{member.name.split(' ')[0]}</span>
                  </div>
                ))
              ) : (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground truncate">{project.client.company_name || project.client.name}</p>
                  </div>
